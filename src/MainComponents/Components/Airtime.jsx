...
const Airtime = () => {
  ...
  const handleDownload = () => {
    ...
    const headers = Object.keys(airtime.data.results[0]).filter((key) => {
      return typeof airtime.data.results[0][key] !== "object";
    });
    ...
    const rows = airtime.data.results
      .map((item) => {
        return headers
          .map((header) => {
    ...
    a.download = `airtimeer_Report_${currentPage}.csv`; // Name the file appropriately
    a.href = url;

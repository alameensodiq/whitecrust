...
import { Cables } from "../Store/Apis/Cables";
...
const Cable = () => {
  ...
  useEffect(() => {
    ...
    dispatch(Cables({ searcher, endDate, startDate, currentPage, download }));
  ...
  const { cables, authenticatingcables } = useSelector((state) => state.cables);
  console.log(cables);

  const next = cables?.data?.next;
  const previous = cables?.data?.previous;
  const totalPosts = cables?.data?.count;
  ...
  const handleDownload = () => {
    ...
    if (cables.data.results.length === 0) {
        ...
        {authenticatingcables ? (
          ...
          <div className="w-[100%] py-9 px-5 flex flex-col gap-10">
           ...
           <div className="flex flex-row justify-end px-8 gap-2">
              ...
              <Tables cables data={cables?.data?.results} />
              <Pagination
                set={activater}

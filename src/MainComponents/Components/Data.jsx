...
import { Datas } from "../Store/Apis/Data";
...
const Data = () => {
  ...
  useEffect(() => {
    ...
    dispatch(Datas({ searcher, endDate, startDate, currentPage, download }));
  ...
  const { datas, authenticatingdatas } = useSelector((state) => state.datas);
  console.log(datas);

  const next = datas?.data?.next;
  const previous = datas?.data?.previous;
  const totalPosts = datas?.data?.count;
        ...
        {authenticatingdatas ? (
          ...
          <div className="w-[100%] py-9 px-5 flex flex-col gap-10">
           ...
           <div className="flex flex-row justify-end px-8 gap-2">
              ...
              <Tables airtime data={datas?.data?.results} />
              <Pagination
                set={activater}

...
import { Audits } from "../Store/Apis/Audits";
...
const Audit = () => {
  ...
  useEffect(() => {
    ...
    dispatch(Audits({ endDate, startDate, currentPage, search, download }));
  ...
  const { audits, authenticatingaudits } = useSelector((state) => state.audits);
  console.log(audits?.data?.results);
  ...
  const handleDownload = () => {
    ...
    console.log("Current accounts state:", audits);

    // Check if accounts and the results array is available
    if (!audits || !audits.data || !Array.isArray(audits.data.results)) {
      toast.error("Investment data is not loaded yet.");
      return;
    }

    if (audits.data.results.length === 0) {
      toast.error("No data available for download.");
      return;
    }

    // Extract headers from the first item in the results, skipping object values
    const headers = Object.keys(audits.data.results[0]).filter((key) => {
      return typeof audits.data.results[0][key] !== "object";
    });
          ...
          <div className="w-[100%] py-9 px-5 flex flex-col gap-10">
            ...
            <div className="flex flex-col border-input-color border-[1px] rounded-custom py-4 gap-6">
              <div className="flex flex-row justify-end gap-4 px-3">
                ...
                <div className="flex flex-row justify-end px-8 gap-2 items-center">
                  <Filter />
                  <span className="text-route-noncolor text-[12px]">
                    Filters
                  </span>
                </div>
              </div>
              <Tables audit data={audits?.data?.results} />
              <Pagination
                set={activater}

...
export const ReportActiveUsers = createAsyncThunk(
  "reportactiveusers",
  async ({ startDateactiveusers }, thunkAPI) => {
    ...
    const dateObj = new Date(startDateactiveusers);
    ...
    try {
      // Construct the URL
      const url = new URL(
        `${process.env.REACT_APP_BASE_URL}superadmin/report?filter=active_users&month=${month}&year=${year}`
      );
